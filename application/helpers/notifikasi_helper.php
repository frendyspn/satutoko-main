...
function notif_tracking_order($kode_transaksi){
    ...
    $message  = "<html><body><span style='font-size:20px'>Informasi Transaksi #$kode_transaksi</span><br>
        ...
        <table style='width:100%'>
            <tbody>";
            $no = 1;
            $record_detail = $ci->db->query("SELECT a.id_penjual, a.kode_transaksi, a.ongkir, a.fee_admin, b.*, c.nama_produk, c.gambar, c.satuan, c.berat, c.produk_seo, d.nama_reseller FROM `rb_penjualan` a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan JOIN rb_produk c ON b.id_produk=c.id_produk JOIN rb_reseller d ON c.id_reseller=d.id_reseller where a.kode_transaksi='".$kode_transaksi."' AND a.kode_kurir!='0'");
    ...
    $isi_pesan = "*$iden[pengirim_email]* - Hai $kons[nama_lengkap]! 
Berikut Informasi Orderan anda #$kode_transaksi : 
...
";
    ...
    $isi_pesan .= "
Ongkos Kirim : Rp ".rupiah($det['ongkir']);
if ($det['fee_admin']>'0'){ $isi_pesan .= "Fee Admin + "; } $isi_pesan .= "Kode : Rp ".rupiah($det['fee_admin']+substr($kode_transaksi,-2))."
...
Status Pesanan : *".strip_tags(status($rowc['proses']))."*";
}		
...
function notif_deposit($nominal,$id_konsumen,$id_rekening){
    ...
    if ($id_rekening=='0'){
        $tujuan_deposit = "Payment Gateway di : ".base_url()."members/withdraw";
        $tujuan_deposit_mail = "<b>Payment Gateway di : ".base_url()."members/withdraw</b>";
    }else{
...
function notif_selesai_belanja($kode_transaksi,$id_konsumen,$total_tagihan_akhir){
    ...
    $co = $ci->db->query("SELECT * FROM rb_penjualan where kode_transaksi='$kode_transaksi'")->row_array();
    $firstname = explode(' ',$kons['nama_lengkap']);
    
    $cek_trx = $ci->db->query("SELECT * FROM rb_penjualan where kode_transaksi='$kode_transaksi'")->num_rows();
    $cek_kurir = $ci->db->query("SELECT * FROM rb_penjualan where kode_transaksi='$kode_transaksi' AND kode_kurir=''")->num_rows();
    
    if ($cek_trx==$cek_kurir){ // Kurir Toko
        $message  = "<html><body><span style='font-size:20px'>Informasi Transaksi #$kode_transaksi</span><br>
                                Hai $kons[nama_lengkap]! Terima kasih telah berbelanja di <a style='text-decoration:none; color:#000' href='$iden[url]'>$iden[url]</a>. Pesanan telah kami terima dan saat ini Driver sedang menuju toko untuk mengambil Pesanan untuk segera diantarkan ke alamat anda.<br><br>
        ...
        $message      .= "<br><span style='color:#999'>Total Tagihan</span><br>
                            <span style='font-size:20px; font-weight:bold; color:red'>Rp ".rupiah($total_tagihan_akhir-substr($kode_transaksi,-2))."</span><br><br>
                            
                            Detail Pengiriman<br>
                            <b>Penerima</b> : $kons[nama_lengkap]<br>
                            <b>No HP</b> : $kons[no_hp]<br>
                            <b>Alamat</b> : ".alamat($kode_transaksi)."<br>
                            
        <br>Untuk melihat Rincian pemesananmu bisa melalui halaman berikut <a style='text-decoration:none; color:green' target='_BLANK' href='".base_url()."konfirmasi/tracking/$kode_transaksi'>".base_url()."konfirmasi/tracking/$kode_transaksi</a>.</body></html> \n";
        
        $isi_pesan = "*$iden[pengirim_email]* - Halo kak $firstname[0], Terima kasih sudah berbelanja di $iden[url], 
        
Pesanan telah kami terima dan saat ini Driver sedang menuju toko untuk mengambil Pesanan untuk segera diantarkan ke alamat anda. sebagai informasi berikut data orderannya : ";
$isi_pesan .= "
...
Alamat : ".alamat($kode_transaksi);

$isi_pesan .= "

Detail pesanan : ".base_url()."t/$kode_transaksi";
...
Penerima : _$kons[nama_lengkap]_
No HP : _$kons[no_hp]_
Alamat : ".alamat_nokor($kode_transaksi)."
...
function notif_penjual($kode_transaksi){
    ...
    foreach ($penjual_notifx->result_array() as $penx) {
        ...
        $cp = $ci->db->query("SELECT * FROM `rb_penjualan_otomatis` where kode_transaksi='$kode_transaksi'")->row_array();
        if ($cp['pembayaran']=='1'){
            $isi_pesan_penjual = "*$iden[pengirim_email]* - Haloo *$penx[nama_reseller]*, ada orderan masuk di toko anda dengan No Invoice #$kode_transaksi dan sudah dibayar LUNAS, yuk segera dikirimkan dan cek detail pesanan di ".base_url()."konfirmasi/tracking/$kode_transaksi";
        }else{
        ...
        if ($cp['pembayaran']=='1'){
            $message_notif = "<html><body><b>$iden[pengirim_email]</b> - Haloo <b>$penx[nama_reseller]</b>, ada orderan masuk di toko anda dengan No Invoice #$kode_transaksi dan sudah dibayar LUNAS, yuk segera dikirimkan dan cek detail pesanan di ".base_url()."konfirmasi/tracking/$kode_transaksi</body></html>";
        }else{
