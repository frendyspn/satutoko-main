    ...
    echo "<div class='col-md-12'>
          ...
          echo "<div class='col-md-12'>
                  ...
                  <tbody>
                    ...
                    <input type='hidden' id='jenis_paket' name='jenis_paket' value='$rows[jenis]'>
                    <tr><th scope='row'>Jenis Paket</th>               <td>"; if ($rows['jenis']=='toko'){ echo "<input type='radio' name='jenis' value='toko' checked> Toko &nbsp; <input type='radio' name='jenis' value='konsumen'> Konsumen"; }else{ echo "<input type='radio' name='jenis' value='toko'> Toko &nbsp; <input type='radio' name='jenis' value='konsumen' checked> Konsumen"; } echo "</td></tr>
                    ...
                    <tr style='display:".($rows['jenis']!='tokok'?'table-row':'none')."' class='jenis'><th width='120px' scope='row'>Max Produk</th>    <td><input type='number' class='form-control' name='max_produk' value='$rows[max_produk]' required></td></tr>
...
$(document).ready(function(){
  $('input:radio[name="jenis"]').change(function(){
    //   $(".jenis").toggle();
      $('#jenis_paket').val($(this).val())
      if ($(this).val() == 'toko') {
        $('input[name="max_produk"]').prop('required',true);
      }else{
        $('input[name="max_produk"]').removeAttr('required');
      }
  });
});
</script>