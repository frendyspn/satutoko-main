            <div class="col-xs-12">  
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Forum Diskusi - Detail</h3>
                  <a class='pull-right btn btn-warning btn-sm' href='<?php echo base_url(); ?>administrator/forum'>Kembali</a>
                </div><!-- /.box-header -->
                <div class="box-body">
                    ...
                    ?>
                        ...
                        <?php 
                            foreach ($komentar->result() as $k) {
                                ...
                                if ($k->aktif_komentar=='N'){ $bg = '#f3d3d3'; $alert = 'default'; $status = 'hidden'; $del1 = '<del>'; $del2 = '</del>'; }else{ $bg = '#f6f8fa'; $alert = 'info'; $status = 'hidden'; $del1 = ''; $del2 = ''; }
                                echo "<table id='komentar-".$k->id_komentar."' style='background:#fff; border-radius:6px; margin-bottom:10px;' class='table table-hover table-condensed'>
                                        <thead>
                                        <tr bgcolor='$bg'>
                                            <td style='padding:10px; text-transform:capitalize' colspan='2'><a style='color:#000' href='#'> <span class='fa fa-user'></span> $del1 $nama $del2</a> <small style='color:#b7b7b7'>commented on ".$this->mylibrary->tgl_indo($tglk[0])." ".$tglk[1]."</small>
                                            <div class='btn-group pull-right' role='group'>
                                                    <button type='button' style='color:#000; background:transparent' class='btn btn-default btn-xs dropdown-toggle' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>
                                                        <span class='fa fa-ellipsis-h'></span>
                                                    </button>
                                                    <ul class='dropdown-menu'>
                                                        <li><a href='".base_url()."administrator/read/".$this->uri->segment('3')."?hide=".$k->id_komentar."&stat=".($k->aktif_komentar=='N'?'Y':'N')."'>Hidden</a></li>
                                                        <li><a href='".base_url()."administrator/read/".$this->uri->segment('3')."?delete=".$k->id_komentar."' onclick=\"return confirm('Apa anda yakin untuk hapus Permanent Data ini?')\">Delete</a></li>
                                                    </ul>
                                                </div>
                                        ...
                                        <tr>
                                            <td valign='top'>$isi_komentar";
                                                ...
                                                $hitungex1 = count($exx);
                                                echo "<div style='padding:5px; margin:0px'  style='padding:5px; margin:0px' class='alert alert-$alert'><i class='fa fa-link fa-fw'></i> Terdapat $hitungex1 File dilampirkan : </div>";
                                                $noi = 1;
...
var settings = {
    url: "<?php echo base_url(); ?>administrator/upload_forum",
...
$("#sendButton").click(function(){
  ...
  $.post('<?php echo base_url(); ?>administrator/sendCommentt', {id: "<?php echo $id ?>", comment: comment})
  .done(function(){
    location.reload();
  });
}); 
