...
class Tripay_payment extends CI_Model{
    ...
    function metode($get="semua"){
        ...
        curl_close($curl);
        if (config('debug_tripay')=='Y'){ print_r(json_decode($response,true)); }
    ...
    function createPayment($trx,$metode,$jumlah,$pembeli,$produk=null){
        ...
        $produk = is_array($produk) ? $produk : [['sku'=>$trx,'name'=>"Transaksi",'price'=> $jumlah,'quantity'=>1]];
        ...
        if(!empty($err)){
            ...
            if($response["success"] != 1){
                ...
                if (count(explode('.',$trx))>1){
                    ...
                    $datas = [
                        "keterangan"=>'TP/'.$response["reference"].'/'.$metode
                    ];
                    $this->db->where("id_withdraw",$trx);
                    $this->db->update("rb_withdraw",$datas);
                }

                $res = array("success"=>true,"pay_code"=>$response["pay_code"],"checkout_url"=>$response["checkout_url"],"msg"=>"Berhasil memproses pembayaran");
                $res = (object)$res;
                return $res;
            }
        }
