	...
	function ulasan(){
		...
		if (isset($_POST['submit'])){
			for ($i=0; $i<count($this->input->post('rating')); $i++) { 
				$cek_orderan = $this->db->query("SELECT a.* FROM rb_penjualan a where a.id_penjualan='".cetak($this->uri->segment(3))."' AND a.id_pembeli='".$this->session->id_konsumen."' AND a.status_pembeli='konsumen'");
 				if ($cek_orderan->num_rows()>=1){
					$cek_ulasan = $this->db->query("SELECT * FROM rb_produk_ulasan where id_penjualan='".cetak($this->uri->segment('3'))."' AND id_produk='".cetak($this->input->post('id_produk')[$i])."'");
					$data = array('id_konsumen'=>$this->session->id_konsumen,
							'id_penjualan'=>cetak($this->uri->segment('3')),
							'id_produk'=>cetak($this->input->post('id_produk')[$i]),
							'rating'=>cetak($this->input->post('rating')[$i]),
							'ulasan'=>(cetak($this->input->post('ulasan')[$i])==''?'Ulasan Otomatis dari System':cetak($this->input->post('ulasan')[$i])),
							'waktu_kirim'=>date('Y-m-d H:i:s'));
					if ($cek_ulasan->num_rows()>0){
						$where = array('id_penjualan'=>cetak($this->uri->segment('3')), 'id_produk'=>cetak($this->input->post('id_produk')[$i]));
						$this->model_app->update('rb_produk_ulasan', $data, $where);
					}else{
						$this->model_app->insert('rb_produk_ulasan',$data);
						notif_ulasan(cetak($this->input->post('id_produk')[$i]),$this->session->id_konsumen,cetak($this->input->post('rating')[$i]));
					}
					
				}
			}
			echo $this->session->set_flashdata('message', "<div class='alert alert-success'><b>BERHASIL</b> - Mengirimkan Ulasan Produk</div>");
			redirect('members/orders_report?page=selesai');
		...
		if (isset($_POST['submit'])){
			...
			if ($cek_orderan->num_rows()>=1){
				if (count($_POST['pilih'])<=0){
				    echo $this->session->set_flashdata('message', "<div class='alert alert-danger'><center><b>GAGAL</b> - Anda belum checklish/Memilih Produk Bermasalah!</center></div>");
					redirect('members/orders_report');
				}else{
					$cek_exist = $this->db->query("SELECT id_pusat_bantuan  FROM rb_pusat_bantuan where id_penjualan='".cetak($this->uri->segment(3))."'");
					if ($cek_exist->num_rows()>=1){
