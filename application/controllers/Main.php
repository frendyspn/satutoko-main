...
class Main extends CI_Controller {
	...
	public function index(){
		...
		$data['maps'] = $maps[0];
		if ($maps[0]!=''){
			$url1 = config('ppob_url').'/v2/pembelian/operator/';
			$header = array(
	...
	function produk(){
        if (isset($_POST['operator_id'])){
            ...
            $url = config('ppob_url').'/v2/pembelian/produk/';
            $header = array(
            'Accept: application/json',
            'Authorization: Bearer '.$maps[0], // Ganti [apikey] dengan API KEY Anda
            );
	...
	function proses(){
		...
		if ($_POST['produk']!=''){
			...
			$urla = config('ppob_url').'/v2/pembelian/produk/';
			$headera = array(
			'Accept: application/json',
			'Authorization: Bearer '.$maps[0], // Ganti [apikey] dengan API KEY Anda
			);
			...
			if (saldo(reseller($this->session->id_konsumen),$this->session->id_konsumen)>=$harga){
				$url = config('ppob_url').'/v2/transaksi/pembelian';
				$header = array(
				'Accept: application/json',
				'Authorization: Bearer '.$maps[0].'', // Ganti [apikey] dengan API KEY Anda
				);
