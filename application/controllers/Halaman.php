...
class Halaman extends CI_Controller {
	public function detail(){
		if ($this->uri->segment('1')=='p'){
			$query = $this->db->query("SELECT * FROM halamanstatis a LEFT JOIN users b ON a.username=b.username where a.id_halaman='".cetak($this->uri->segment(2))."'");
		}else{
		...
		if ($query->num_rows()<=0){
			...
			if ($row['landing_page']=='Y'){
				$this->load->view(template().'/detailhalaman_landing',$data);
			}else{
